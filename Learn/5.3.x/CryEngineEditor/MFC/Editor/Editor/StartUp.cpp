...
#include <CryCommon/CryGame/IGameFramework.h>
...
int StartUp::RunGame(const char* commandLine,HWND hwnd)
{
...
int StartUp::CryWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow,HWND hwnd)
{
	...
	if (pos != NULL)
	{
		...
		return RunGame(lpCmdLine,hwnd); //pass the restart level if restarting
	}
	else
		pos = strstr(lpCmdLine, " -load ");// commandLine.find("load");

	if (pos != NULL)
		RunGame(lpCmdLine,hwnd);

	int nRes = RunGame(GetCommandLineA(),hwnd);
